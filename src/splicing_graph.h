...
extern std::set<pair_t> Inhibit_edges;
  ...
  std::string describe_graph () {
    ...
    for (size_t i = 0; i < size_; ++i) {
      ...
      for (size_t j = 0; j < node_set_[i].children.size(); ++j) {
        ...
        std::string edge =  get_edge_sequence(i, node_set_[i].children[j]);
        graph << ":" << compute_coverage(edge);
    ...
    //------------------------------  Add New Lines Begin  ------------------------------------
    graph << "** Inhibit_edges **" << std::endl; 
    std::set<pair_t>::iterator it;
    for (it = Inhibit_edges.begin() ; it != Inhibit_edges.end();++it) {
      graph << (*it).first << "->" << (*it).second<<";"<< std::endl;
    }
    graph << "** Pair_edges **" << std::endl;
    for (int it=0;it<Pair_edges.size();it++)
    {
    	graph << Pair_edges[it][0] << "->" <<Pair_edges[it][1] << "->" <<Pair_edges[it][2] << std::endl;
    }	
    //------------------------------  Add New Lines End  --------------------------------------
