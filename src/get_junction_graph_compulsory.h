#ifndef GET_JUNCTION_GRAPH_COMPULSORY
#define GET_JUNCTION_GRAPH_COMPULSORY

//get_junction_graph_compulsory.h
...
class Get_Junction_Graph_
{
	private:
	char * path;
	public:
	Get_Junction_Graph_(char * p);
	~Get_Junction_Graph_();
...
Get_Junction_Graph_::Get_Junction_Graph_(char * p)
{
	path=p;
	return;
}
Get_Junction_Graph_::~Get_Junction_Graph_()
{
	return;
}
void Get_Junction_Graph_::Construct_Junction_Graph()
{
	ifstream ifs(path);
	string temp;
	...
	for (i=0;i<=max_num;i++)
	{
		...
		if (temp_in.size()>=1 && temp_out.size()>=1)
		{
			...
			{
				cov_out=cov_out+DAG_weights[temp_out[j]];
			}
			for (j=0;j<temp_in.size();j++)
			{
				cov_in=cov_in+DAG_weights[temp_in[j]];
			}
			if (cov_out>=10 && cov_in>=1 && cov_out/cov_in>=2)
			{
				DAG_left.push_back(max_node_num+1);
				DAG_right.push_back(i);
				DAG_weights.push_back(cov_out-cov_in);
			}
			if (cov_in>=10 && cov_out>=1 && cov_in/cov_out>=2)
			{
				DAG_left.push_back(i);
                                DAG_right.push_back(max_node_num+2);
                                DAG_weights.push_back(cov_in-cov_out);
			}
		}
	}
	pos_left=max_element(DAG_left.begin(),DAG_left.end());
